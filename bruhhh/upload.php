...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $email = $_POST['email'];
    $sql1 = "SELECT u.user_id as user_id, s.supplier_id as s_id, u.email 
    from user u, supplier s where u.email = '$email' and u.user_id = s.user_id";
    $result1 = $conn->query($sql1);
    $row = $result1->fetch_assoc();
    $supplier_id = $row['s_id'];
    $sql =  "INSERT INTO  item(item_id, item_name, item_rating, quantity, price, supplier_id, item_desc) 
            VALUES (NULL, '$item_name','0', '$quantity', '$item_price', '$supplier_id', '$item_desc')";
